...
def get_beta_expansion(l, D, l0=1550, polyOrder=10, return_everything=True):
...
for axrow, thick in zip(axs, thicknesses):
    ...
    for filename in glob.glob(folder+'/t_%inm/*.mat'%thick): # skipping every second width
        ...
        for num, b in enumerate(beta_coefs[:9]):
